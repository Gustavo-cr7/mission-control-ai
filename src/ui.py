...
def show_banner():
    ...
    console.print(Panel.fit(
        ...
        title="◆ ENVIROSAT CONTROL", border_style="#06B6D4"
    ))

def show_response(text):
    """Renderiza resposta da IA em painel com timestamp."""
    now = datetime.now().strftime("%H:%M")
    console.print(Panel(text, title="◆ EnviroSat Mission Control",
...
def run_cli(engine):
    ...
    while True:
        ...
        if user_input == "/about":
            show_response(
                "EnviroSat é uma missão simulada de observação ambiental.\n"
                "O sistema monitora telemetria do satélite, identifica alertas "
                "e usa IA generativa para explicar o impacto ambiental na Terra."
            )
            continue
